...
struct CustomHomeShape: Shape {
    func path(in rect: CGRect) -> Path {
        ...
        let w = rect.width
        let h = rect.height
        let roofBottom = h * 0.45 // Matches the slope angle of your image
        
        path.move(to: CGPoint(x: 0, y: h))                  // Bottom left
        path.addLine(to: CGPoint(x: 0, y: roofBottom))      // Straight up left wall
        path.addLine(to: CGPoint(x: w / 2, y: 0))           // Angled up to roof peak
        path.addLine(to: CGPoint(x: w, y: roofBottom))      // Angled down to right wall
        path.addLine(to: CGPoint(x: w, y: h))               // Straight down right wall
        path.closeSubpath()                                 // Close bottom line
